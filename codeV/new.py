...
def get_dense_points_from_mesh(mesh, target_density=80000):
    """
    Robustly generate dense points from mesh using subdivision if needed.
    Falls back gracefully if subdivision fails.
    """
    vertices = np.asarray(mesh.vertices)
    current_count = len(vertices)
    
    # If we already have enough vertices, use them directly
    if current_count >= target_density * 0.5:  # Within 50% is good enough
        print(f"✅ Using {current_count} mesh vertices directly")
        return vertices
    
    # Try to subdivide to increase density
    print(f"⚠️  Mesh has only {current_count} vertices. Attempting subdivision...")
    try:
        subdivided_mesh = mesh.subdivide_midpoint(number_of_iterations=1)
        vertices = np.asarray(subdivided_mesh.vertices)
        print(f"✅ Subdivision successful: {len(vertices)} vertices")
        
        # If still not enough after one iteration, try one more
        if len(vertices) < target_density * 0.5:
            subdivided_mesh = subdivided_mesh.subdivide_midpoint(number_of_iterations=1)
            vertices = np.asarray(subdivided_mesh.vertices)
            print(f"✅ Second subdivision: {len(vertices)} vertices")
        
        return vertices
    
    except Exception as e:
        print(f"⚠️  Subdivision failed: {e}")
        print(f"⚠️  Falling back to uniform sampling...")
        # Fallback to sampling if subdivision fails
        np.random.seed(42)  # Deterministic sampling
        pcd = mesh.sample_points_uniformly(number_of_points=target_density)
...
def detect_spinal_midline_projection_method(
    ...
    if allPoints is None:
        allPoints = get_dense_points_from_mesh(backMesh, target_density=80000)
    ...
    print("\n" + "="*70)
    print("SPINAL MIDLINE DETECTION (V8.1 - Robust Point Generation)")
    print("="*70)
...
def save_top_down_view(slice_data, filename):
    ...
    valley_sphere = o3d.geometry.TriangleMesh.create_sphere(radius=2.5)
...
def main():
    ...
    # Generate dense points robustly
    allPoints = get_dense_points_from_mesh(backMesh, target_density=80000)
    
    # Detect spinal midline with the generated points
    ...
    o3d.visualization.draw_geometries(
        vis_geometries,
        window_name="SPINAL MIDLINE V8.1 (Robust - Final 3D View)",
    )
